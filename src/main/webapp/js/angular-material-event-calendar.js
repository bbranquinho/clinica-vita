...
(function(){"use strict";angular.module("material.components.eventCalendar").run(["$templateCache", function($templateCache) {$templateCache.put("icons/close.svg","<svg fill=\"#000000\" height=\"24\" viewBox=\"0 0 24 24\" width=\"24\" xmlns=\"http://www.w3.org/2000/svg\">\n    <path d=\"M19 6.41L17.59 5 12 10.59 6.41 5 5 6.41 10.59 12 5 17.59 6.41 19 12 13.41 17.59 19 19 17.59 13.41 12z\"/>\n    <path d=\"M0 0h24v24H0z\" fill=\"none\"/>\n</svg>");
...
function mdEventCalendarBuilderService($$mdEventCalendarUtil, $templateCache) {
  ...
  function showMore(opts) {
    ...
    content.setAttribute('style','max-height: 100px;')
    ...
    closeButton.innerHTML = $templateCache.get('icons/close.svg');
  ...
  function month(options) {
    ...
    function getCellOptions(cellDate, dayOfWeek, differentMonth) {
      return {
        ...
        maxEvents: 0, // max events that can be displayed in a day cell. based on cell size
  ...
  function createCellElement(options) {
   ...
   // if (options.showCreateLink === true) {
      ...
      /*var iconCreateLink = document.createElement('i');
      iconCreateLink.setAttribute('md-create-event', '');
      //iconCreateLink.classList.add('md-event-calendar-create-link');
      iconCreateLink.classList.add('material-icons');
      iconCreateLink.textContent= 'rate_review';*/


      //<i class='material-icons'>rate_review</i>
     createLink.textContent = '';
      //createLink.appendChild(iconCreateLink);
      cellHeader.appendChild(createLink);
    //}

    createEventElements(cellHeader,cellContent, options);
  ...
  function createEventElements(cellHeader,cellContent, options) {
    var i;
    var place = 0;
    var hasEvents = false;
    ...
    matchingEvents.every(function (eventItem, pos) {
     ...
     // console.log(`place: ${place} - options: ${options.maxEvents}`);
      if (place == options.maxEvents -1 || place >= options.maxEvents) {
        //cellContent.appendChild(createEventElement(type, eventItem, options));
       // console.log(`pos: ${pos}`);
        cellContent.appendChild(createShowMore(matchingEvents.length - pos+1, options.date));
        return false;
      }
      //cellContent.appendChild(createEventElement(type, eventItem, options));
      return true;
  ...
  function createShowMore(num, date) {
    ...
    var iconCreateMore = document.createElement('i');
    iconCreateMore.classList.add('material-icons');
    iconCreateMore.textContent= 'remove_red_eye';
    iconCreateMore.setAttribute('md-show-more', date.toISOString());


   // showMoreElement.textContent = num+' +';
    showMoreElement.setAttribute('md-show-more', date.toISOString());
    showMoreElement.appendChild(iconCreateMore);
...
function eventCalendarMonthDirective($$mdEventCalendarBuilder, $window, $$rAF, $timeout) {
  ...
  function link(scope, element, attrs, ctrl) {
    ...
    function positonShowMore() {
      ...
      if (leftDiff > 0) {
        showMoreData.element.style.left = (showMoreData.element.style.left.split('p')[0] - 200)+'px';
        console.log(showMoreData.element.style.left.split('p')[0]);
        // offset date
