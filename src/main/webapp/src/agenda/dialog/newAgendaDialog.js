...
angular.module('clinica')
    ...
    .controller('NewAgendaDialogController',function ( $mdDialog, $scope, $mdMedia,FormatDate, $mdToast,RestSrv,SERVICE_PATH,_events, _dialogData,_medico) {
       ...
       // alert($scope.calendarEvent);
        ...
        $scope.formattimepiker = function(fieldData,time) {
           ...
           /* if( $scope.itemAgenda === undefined){
                $scope.itemAgenda = {};
                $scope.itemAgenda.agenda = {};
            }


            if(fieldData === 'dataInicialConsulta'){
                console.log( $scope.horaInicial);
                $scope.itemAgenda.agenda.dataHoraInicialConsulta = FormatDate.format($scope.dataInicialConsulta) + $scope.horaInicial+ $scope.horaInicial;

            }
            if(fieldData === 'dataHoraInicial'){
                console.log($scope.horaInicial);
       ...
       // init();
        ...
        /*function init()
        {
            ...
            // Edit
            if ( $scope.dialogData.calendarEvent )
            {
                // Clone the calendarEvent object before doing anything
                // to make sure we are not going to brake FullCalendar
                $scope.calendarEvent = angular.copy($scope.dialogData.calendarEvent);

                // Convert moment.js dates to javascript date object
                if ( moment.isMoment($scope.calendarEvent.start) )
                {
                    $scope.calendarEvent.start = $scope.calendarEvent.start.toDate();
                }

                if ( moment.isMoment($scope.calendarEvent.end) )
                {
                    $scope.calendarEvent.end = $scope.calendarEvent.end.toDate();
                }
            }
            // Add
            else
            {
                // Convert moment.js dates to javascript date object
                if ( moment.isMoment($scope.dialogData.start) )
                {
                    $scope.dialogData.start = $scope.dialogData.start.toDate();
                }

                if ( moment.isMoment($scope.dialogData.end) )
                {
                    $scope.dialogData.end = $scope.dialogData.end.toDate();
                }

                $scope.calendarEvent = {
                    start        : $scope.dialogData.start,
                    end          : $scope.dialogData.end,
                    notifications: []
                };
            }
        }*/
